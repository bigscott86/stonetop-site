...
## Weather & night (built 2026-09-10; textures and viewport layers 2026-09-10)
...
stay visible to everyone. The effects show on every device.

**Where the layers live matters.** `#weather` is a sibling of `#map-inner` (on the viewport,
above the map, `pointer-events:none`). A first version put it inside `#map-inner` so it would
pan and zoom with the parchment, but a map-sized layer at retina scale or zoomed in exceeds what
the GPU composites smoothly — the rain flickered and menus blinked while skies changed. Now every
moving layer is **viewport-sized but pinned to the map**: `wxLayout()` (called from `applyView`)
places each tiled texture (`.wx-m` outer, `<i>` inner) so its origin is the map origin modulo one
tile — `ox=((MV.tx%Tw)+Tw)%Tw-Tw` — and its tile `Tw=tw*s` scales with the zoom up to a cap
(`min(MV.s, fit*1.6)`). The outer is sized viewport + one or two tiles of margin per motion
...
noise on the parchment), `wx-fog1/2` (noise sheets rolling opposite ways; fog2 faintly under
rain), `wx-night` (moonlit blue multiply) + `wx-vig` (edge vignette). Textures are SVG data URIs
generated by the scratch patch script (seeded random streaks/flakes; `feTurbulence` with the blur
baked in). **Gotcha:** inside an SVG data URI every `#` must be `%23` (`filter='url(%23f)'`) — a
raw `#` ends the URI and the noise sheets rendered as nothing. The modal backdrop blur is off
unless the sky is clear (it re-reads the animated layers every frame). Animations stop under
...
- **Panel polish (2026-09-10).** Chapter-style tabs (`.tabs .tab`/`.stab` in the display face),
  a button order (filled gold `.sm-btn-go`/`.btn-primary` = the one primary action per panel,
  outlined `.btn`/`.cc-btn`/`.pk-btn` secondaries), **ledger inputs** (single-line inputs in
  panels are underline-only; textareas and selects keep a box), a gold diamond before each section
  head, thin scrollbars, empty states that carry their action (`[data-bempty-edit]` toggles Edit;
  the roster's empty state has its own New character button), and **jump bars**: `jumpBar(container,
  '.cc-sec')` builds a sticky row of section chips at the top of the Village Sheet and the character
  editor (`top:-24px` because Chrome measures sticky from the content box; `.cc-sec` has
  `scroll-margin-top`).
