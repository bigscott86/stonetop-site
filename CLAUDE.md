...
## Weather & night (built 2026-09-10, textures 2026-09-10)
...
parchment and all sizes are map-image pixels (the fitted view shows them at about half size).
Layers, each a `.wx-*` div whose opacity is set by a class-only rule (an id selector once
pinned them all at 0): `wx-tint` (a multiply gradient per weather), `wx-cloud` (fractal-noise
cloud shadows drifting, multiply; overcast/rain/storm/snow), `wx-rain1..3` (three depths of
generated SVG streak tiles, faster and denser in a storm), `wx-flash1/2` (two radial lightning
strikes on unrelated timers), `wx-snow1..3` (generated flake tiles that sway as they fall) +
`wx-frost` (screen-blended noise settling on the parchment), `wx-fog1/2` (two noise sheets
rolling opposite ways; a faint one also under rain), `wx-night` (moonlit blue multiply) +
`wx-vig` (edge vignette). The textures are SVG data URIs generated by the scratch patch script
(seeded random streaks/flakes, `feTurbulence` noise with the blur baked in); everything moves
by `transform` over tile-aligned loops so each active layer paints once and only composites per
frame. Animations stop under `prefers-reduced-motion`. **Gotcha:** inside an SVG data URI every `#`
must be `%23` (`filter='url(%23f)'`) — a raw `#` ends the URI, and the noise sheets rendered as
nothing for a while because of it. The fog is tuned to banks (alpha contrast in the noise matrix,
layer opacities .8/.65) with the village still readable between them.
